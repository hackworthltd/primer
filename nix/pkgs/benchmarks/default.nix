...
let
  ...
  primer-benchmark-results-html = (runCommand "primer-benchmark-results-html" { }
    ''
      ...
      ${primer-benchmark}/bin/primer-benchmark --output $out/results.html --regress allocated:iters --regress numGcs:iters
    ''
  ).overrideAttrs
    (drv: {
  ...
  primer-benchmark-results-json = (runCommand "primer-benchmark-results-json" { }
    ''
      ${coreutils}/bin/mkdir -p $out
      cp ${lastEnvChangeFile} $out/lastEnvChange
      ${primer-benchmark}/bin/primer-benchmark --template json --output $out/results.json --regress allocated:iters --regress numGcs:iters
    ''
  ).overrideAttrs
    (drv: {
      requiredSystemFeatures = (drv.requiredSystemFeatures or [ ]) ++ [ "benchmark" ];
