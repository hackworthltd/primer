...
import Primer.Core (
  ...
  _exprKindMeta,
  _exprMeta,
  _exprTypeMeta, CaseFallback' (CaseExhaustive, CaseFallback), caseBranchName,
 )
...
alphaEq = go (0, mempty, mempty)
  where
    ...
    go bs (Letrec _ v1 t1 ty1 e1) (Letrec _ v2 t2 ty2 e2) = go (newTm bs v1 v2) t1 t2
          && alphaEqTy' (extractTypeEnv bs) ty1 ty2
          && go (newTm bs v1 v2) e1 e2
    go bs (Case _ e1 brs1 fb1) (Case _ e2 brs2 fb2) = go bs e1 e2
       && and (zipWith (\(CaseBranch c1 (fmap bindName -> vs1) t1)
