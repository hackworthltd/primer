...
import Primer.Core (LVarName, Type' (TForall, TFun, TVar))
import qualified Primer.Core as C
import Primer.Core.Utils (freshLVarName)
...
data Inst
  = InstApp TC.Type
  | InstAPP TC.Type
  | InstUnconstrainedAPP LVarName C.Kind
  deriving (Show, Eq)
...
refine cxt tgtTy srcTy = go [] srcTy
  where
    ...
    go :: [Either TC.Type (LVarName, C.Kind)] -> TC.Type -> m (Maybe ([Inst], TC.Type))
