...
import Primer.Zipper.Type (
  LetTypeBinding,
  LetTypeBinding' (LetTypeBind), getBoundHereDnTy, letTypeBindingName,
 )
import Data.Generics.Uniplate.Data (children, descendM, descendBiM)
...
data Redex
  ...
  | -- let x = e in x  ~>  e
    InlineLet -- TODO: don't know if I want to change this def for push-down-lets, as only inline an immediate usage
      { var :: LVarName
  ...
  | -- letrec x = t : T in x  ~>  letrec x = t : T in t : T
    InlineLetrec -- TODO: don't know if I want to change this def for push-down-lets, as only inline an immediate usage
      { var :: LVarName
      -- ^ What variable are we inlining
      , expr :: Expr
      -- ^ What is its definition
  ...
  = -- let a = t in a  ~>  t
    InlineLetInType
      { -- | What variable are we inlining (used for finding normal-order redex)
        ...
        ty :: Type,
        -- | Where was the binding (used for details)
        letID :: ID, -- TODO/REVIEW: it seems a bit silly recording both these IDs, since they'll be right next to each other...
        -- | Where was the occurrence (used for details)
        varID :: ID
      }
...
viewRedex tydefs globals dir = \case
  ...
  orig@(viewLets -> Just (bindings, expr))
    ...
    , allLetsUsed (fmap snd bindings) expr ->
      ...
      where
        ...
        allLetsUsed ls b = let used = filterLets ls b in length used == length ls
  ...
  orig@(Letrec _ v e1 t body)
    | Var _ (LocalVarRef w) <- body, v == w -> pure $ InlineLetrec {
        var=v, expr=e1, ty=t, letID=getID orig, varID=getID body}
...
filterLets' ls fvs = fst $ foldr (\l (ls',fvs') -> let ln = letBindingName l  in if ln `S.member` fvs'
...
runRedexTy (PushLetType {bindings, intoTy, origTy, bindingIDs}) = do
  ty' <- descendM (addTLets $ LetTyBind <$> bindings) intoTy
  let details =
        PushLetDetail
          { before = origTy,
