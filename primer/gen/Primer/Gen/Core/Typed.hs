          ...
          then Nothing
            ...
            runExceptT (refine cxt ty hT) >>= \case
              ...
              Right (Just (inst, instTy)) -> do
                ...
                Just . (foldl' f he is,) <$> substTyIter (M.toList sb) instTy
    genApp = do
      s <- genWTType KType
...
genInstApp = reify mempty
  where
    reify sb = \case
      [] -> pure (sb, [])
      InstApp t : is -> (\a -> second (Right a :)) <$> (substTyIter (M.toList sb) t >>= genChk) <*> reify sb is
      InstAPP t : is -> (\t' -> second (Left t' :)) <$> substTyIter (M.toList sb) t <*> reify sb is
