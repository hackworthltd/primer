...
hprop_con = propertyWTInExtendedLocalGlobalCxt defaultCxt $ do
  ...
  annotateShow src
  tgt' <- forAllT $ traverse (genWTType . snd) $ typeDefParameters td
  let tgt = foldl (TApp ()) (TCon () $ typeDefName td) tgt'
  annotateShow tgt
  cxt <- ask
  r <- refine' cxt tgt src
  case r of
...
hprop_arr_app = propertyWTInExtendedLocalGlobalCxt defaultCxt $ do
  ...
  src' <- forAllT $ Gen.list (Range.linear 0 10) $ Gen.choice [Left <$> genWTType KType, curry Right <$> freshNameForCxt <*> genWTKind]
