...
runTryReduce :: TypeDefMap -> DefMap -> Locals -> (Expr, ID) -> Either EvalError (Expr, EvalDetail)
runTryReduce tys globals locals (expr, i) = evalTestM i $ runExceptT $ tryReduceExpr tys globals locals Syn expr
