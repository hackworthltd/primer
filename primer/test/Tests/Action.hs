...
unit_refine_4 =
  actionTest
    NoSmartHoles
    (let_ "nil" (lAM "a" $ con cNil [tvar "a"] []) $ emptyHole `ann` (tcon tList `tapp` tcon tNat))
    [Move Child2, Move Child1, InsertRefinedVar $ LocalVarRef "nil"]
    (let_ "nil" (lAM "a" $ con cNil [tvar "a"] []) $ (lvar "nil" `aPP` tcon tNat) `ann` (tcon tList `tapp` tcon tNat))

unit_refine_5 :: Assertion
unit_refine_5 =
  actionTest
    NoSmartHoles
    (let_ "nil" (lAM "a" $ con cNil [tvar "a"] []) $ emptyHole `ann` (tcon tList `tapp` tEmptyHole))
    [Move Child2, Move Child1, InsertRefinedVar $ LocalVarRef "nil"]
    (let_ "nil" (lAM "a" $ con cNil [tvar "a"] []) $ (lvar "nil" `aPP` tEmptyHole) `ann` (tcon tList `tapp` tEmptyHole))
