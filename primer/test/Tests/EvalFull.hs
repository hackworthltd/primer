...
unit_type_preservation_BETA_regression =
  let (((exprA, expectedAs), (exprB, expectedBs)), maxID) = create $ do
        ...
        -- not "concrete"
        pure
          ( (eA, NE.zip [1..] [expectA1, expectA2, expectA3, expectA4, expectA5, expectA6, expectA7,
                  expectA8,expectA9,expectA10])
              ...
              lam "f" $
                lam "x" $
                  (letrec "x" (lvar "f" `app` lvar "x") (tvar "a") (lvar "f")
                   `app` letrec "x" (lvar "f" `app` lvar "x") (tvar "a") (lvar "x")) `ann` tvar "a"
          expect4d <-
            lAM "a" $
              lam "f" $
                lam "x" $
