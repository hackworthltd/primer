...
unit_11 =
  let modName = mkSimpleModuleName "TestModule"
      ((globals, e, expected), maxID) = create $ do
        ...
        pure (globs, expr, expect)
   in do
        s <- evalFullTestExactSteps maxID builtinTypes (M.fromList globals) 19 Syn e
        s ~== expected
...
unit_12 =
  let ((e, expected), maxID) = create $ do
        ...
        pure (expr, expect)
   in do
        s <- evalFullTestExactSteps maxID builtinTypes mempty 14 Syn e
        s ~== expected
...
unit_letrec_body_first =
  let lx = let_ "x" ((lam "x" (lvar "x") `ann` (tcon tBool `tfun` tcon tBool)) `app` con0 cTrue)
      ...
      expected3 = create' $ (con cCons [lx $ lxs $ lvar "x", lx $ lxs $ lvar "xs"]) `ann` (tcon tList `tapp` tEmptyHole)
   in do
